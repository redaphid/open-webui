...
from open_webui.config import (
    ...
    CODE_INTERPRETER_DAEMON_PROMPT,
...
from open_webui.utils.code_mode import generate_code_mode_prompt, generate_mcp_bindings
...
async def process_chat_payload(request, form_data, user, metadata, model):
        ...
        if 'code_interpreter' in features and features['code_interpreter']:
            ...
            if metadata.get('params', {}).get('function_calling') == 'legacy':
                ...
                if engine != 'jupyter':
                    ...
                    # Only the jupyter engine can host persistent background
                    # daemons; pyodide runs in the browser and cannot.
                    prompt += CODE_INTERPRETER_DAEMON_PROMPT
    ...
    if payload_tools is None:
        ...
        if mcp_tools_dict:
            ...
            # Add code mode prompt if Jupyter code interpreter is enabled with MCP
            # tools (Pyodide runs in the browser sandbox and cannot reach the MCP
            # proxy, so bindings would be unreachable there).
                    ...
                    while output and output[-1].get('type') == 'open_webui:code_interpreter' and retries < MAX_RETRIES:
                        ...
                        try:
                            if ci_item.get('attributes', {}).get('type') == 'code':
                                ...
                                # Code Mode: inject MCP tool bindings so the
                                # interpreter can call MCP tools as plain Python
                                # functions via the internal proxy endpoint.
                                code_mode_session_id = None
                                mcp_tools = metadata.get('mcp_tools', {})
                                if mcp_tools and ci_engine == 'jupyter':
                                    ...
                                    if mcp_bindings:
                                        ...
                                        log.debug(
                                            f'Injected MCP bindings for code mode session: {code_mode_session_id}'
                                        )

                                is_background = ci_item.get('attributes', {}).get('background') == 'true'

                                if is_background and ci_engine == 'jupyter':
                                    # Background daemon execution. Output streams to
                                    # the client over Socket.IO rather than through
                                    # this response.
                                    try:
                                        ...
                                        daemon_id = await daemon_executor.start_daemon(
                                            base_url=await Config.get('code_interpreter.jupyter.url'),
                                            code=code,
                                            token=(
                                            ...
                                            max_runtime=int(
                                                await Config.get('code_interpreter.daemon.max_runtime', 3600)
                                            ),
                                        )
                                        ci_output = {
                                        ...
                                        log.exception(f'Failed to start background script: {daemon_err}')
                                        ci_output = {
