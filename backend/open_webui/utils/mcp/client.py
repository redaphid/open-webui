...
class MCPClient:
    ...
    async def _ensure_connected(self):
        ...
        Long-lived consumers (code mode bindings, background daemons) hold a
        client across many turns, by which point the transport may have gone
        away. Re-establish it transparently rather than failing the call.
        """
    ...
    async def call_tool(self, function_name: str, function_args: dict) -> Optional[dict]:
        ...
        if result.isError:
            # Surface the tool's own text content rather than the raw dump.
    ...
    async def __aenter__(self):
        """Enter the context. connect() must be called separately.

        Note: exit_stack does not exist until connect() succeeds, so there is
        nothing to enter here.
        """
        return self

    async def __aexit__(self, exc_type, exc_value, traceback):
        """Exit the context, closing the session if one was established."""
        await self.disconnect()
